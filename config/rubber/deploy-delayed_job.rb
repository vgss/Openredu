namespace :rubber do
  ...
  namespace :delayed_job do
    ...
    def app_queues
      rubber_env.delayed_job_queues.each_with_index do |queue, count|
        yield "--queue=#{queue} -i #{count}"
      end
    end

    desc "Stop the delayed_job process"
    task :stop, :roles => :delayed_job do
      self.app_queues do |queue_args|
        rsudo "cd #{current_path} && RAILS_ENV=#{Rubber.env} bundle exec #{self.script_path} stop #{self.args} #{queue_args}", :as => rubber_env.app_user
      end
    end
    ...
    task :start, :roles => :delayed_job do
      self.app_queues do |queue_args|
        rsudo "cd #{current_path} && RAILS_ENV=#{Rubber.env} bundle exec #{self.script_path} start #{self.args} #{queue_args}", :as => rubber_env.app_user
      end
    end
    ...
    task :restart, :roles => :delayed_job do
      self.app_queues do |queue_args|
        rsudo "cd #{current_path} && RAILS_ENV=#{Rubber.env} bundle exec #{self.script_path} restart #{self.args} #{queue_args}", :as => rubber_env.app_user
      end
    end
